...
// npm's code on disk lives in the parent experiment's node_modules.
const NPM_NM = path.join(HARNESS_DIR, "..", "node_modules");
...
`  #extract (dest, tarball) {
  ...
  // WORKAROUND (cacache content-write hang): when pacote installs WITH a cache,
  // the tarball stream is tee'd into cacache's content-write pipeline, which uses
  // fs-minipass \`new WriteStream(tmpfile, {flags:'wx'})\` — the ASYNC variant
  // (fs.open -> fs.write -> fs.close callbacks). Under workerd those async fs
  // completion callbacks are intermittently lost when many such streams run
  // concurrently (~100 tarballs each open their own async WriteStream), so the
  // Pipeline promise never resolves, cacache's handleContent() never settles, the
  // tee backpressures, and Arborist.reify() hangs FOREVER with 0 files written and
  // 0 files in the cache (the tmp file is never closed/moved). This is the SAME
  // class as the already-fixed tar async-write hang. Force fs-minipass's async
  // WriteStream onto the SYNCHRONOUS code path (openSync/writeSync/closeSync) — the
  // exact ops WriteStreamSync already uses and which extract proved reliable. The
  // stream's external async contract (events) is preserved; only the fs ops are sync.
  if (virtPath.endsWith("/fs-minipass/lib/index.js") && process.env.FSM_SYNC !== "0") {
    src = src.replace(
      "    this[_writing] = true\n    this[_write](buf)\n    return true\n  }\n\n  [_write] (buf) {\n    fs.write(this[_fd], buf, 0, buf.length, this[_pos], (er, bw) =>\n      this[_onwrite](er, bw))\n  }",
      "    this[_writing] = true\n    this[_write](buf)\n    return true\n  }\n\n  [_write] (buf) {\n    // workerd: synchronous write (async fs.write callbacks are lost at volume)\n    try { const bw = fs.writeSync(this[_fd], buf, 0, buf.length, this[_pos]); queueMicrotask(() => this[_onwrite](null, bw)); }\n    catch (er) { queueMicrotask(() => this[_onwrite](er)); }\n  }"
    );
    src = src.replace(
      "      const iovec = this[_queue]\n      this[_queue] = []\n      writev(this[_fd], iovec, this[_pos],\n        (er, bw) => this[_onwrite](er, bw))",
      "      const iovec = this[_queue]\n      this[_queue] = []\n      // workerd: synchronous writev (async callbacks are lost at volume)\n      try { const bw = fs.writevSync(this[_fd], iovec, this[_pos]); queueMicrotask(() => this[_onwrite](null, bw)); }\n      catch (er) { queueMicrotask(() => this[_onwrite](er)); }"
    );
    src = src.replace(
      "  [_close] () {\n    if (this[_autoClose] && typeof this[_fd] === 'number') {\n      const fd = this[_fd]\n      this[_fd] = null\n      fs.close(fd, er => er ? this.emit('error', er) : this.emit('close'))\n    }\n  }\n}",
      "  [_close] () {\n    if (this[_autoClose] && typeof this[_fd] === 'number') {\n      const fd = this[_fd]\n      this[_fd] = null\n      // workerd: synchronous close (async fs.close callback is lost at volume)\n      try { fs.closeSync(fd); queueMicrotask(() => this.emit('close')); }\n      catch (er) { queueMicrotask(() => this.emit('error', er)); }\n    }\n  }\n}"
    );
    // The async WriteStream's _open is also async (fs.open). Make it synchronous too,
    // so the whole open->write->close chain has no async fs completion callbacks.
    src = src.replace(
      "  [_open] () {\n    fs.open(this[_path], this[_flags], this[_mode],\n      (er, fd) => this[_onopen](er, fd))\n  }",
      "  [_open] () {\n    // workerd: synchronous open (async fs.open callback is lost at volume)\n    try { const fd = fs.openSync(this[_path], this[_flags], this[_mode]); queueMicrotask(() => this[_onopen](null, fd)); }\n    catch (er) { queueMicrotask(() => this[_onopen](er)); }\n  }"
    );
    // --- ReadStream sync path -------------------------------------------------
    // cacache.get.stream.byDigest() (used by pacote's #extract on every tarball)
    // returns an fs-minipass ReadStream whose _read() issues async fs.read calls.
    // Those completion callbacks are ALSO lost under workerd at volume, so the
    // ReadStream never emits 'end', and the (already-sync) tar extraction waits on
    // tarball.on('end') FOREVER. THIS is the dominant hang once the WriteStream
    // path is sync. Force ReadStream's open/read onto synchronous fs ops too.
    src = src.replace(
      "  [_open] () {\n    fs.open(this[_path], 'r', (er, fd) => this[_onopen](er, fd))\n  }",
      "  [_open] () {\n    // workerd: synchronous open (async fs.open callback is lost at volume)\n    try { const fd = fs.openSync(this[_path], 'r'); queueMicrotask(() => this[_onopen](null, fd)); }\n    catch (er) { queueMicrotask(() => this[_onopen](er)); }\n  }"
    );
    src = src.replace(
      "      fs.read(this[_fd], buf, 0, buf.length, null, (er, br, b) =>\n        this[_onread](er, br, b))",
      "      // workerd: synchronous read (async fs.read callback is lost at volume)\n      try { const br = fs.readSync(this[_fd], buf, 0, buf.length, null); queueMicrotask(() => this[_onread](null, br, buf)); }\n      catch (er) { queueMicrotask(() => this[_onread](er)); }"
    );
    // ReadStream._close (identical body to WriteStream's, but followed by [_onerror]).
    src = src.replace(
      "  [_close] () {\n    if (this[_autoClose] && typeof this[_fd] === 'number') {\n      const fd = this[_fd]\n      this[_fd] = null\n      fs.close(fd, er => er ? this.emit('error', er) : this.emit('close'))\n    }\n  }\n\n  [_onerror] (er) {",
      "  [_close] () {\n    if (this[_autoClose] && typeof this[_fd] === 'number') {\n      const fd = this[_fd]\n      this[_fd] = null\n      // workerd: synchronous close (async fs.close callback is lost at volume)\n      try { fs.closeSync(fd); queueMicrotask(() => this.emit('close')); }\n      catch (er) { queueMicrotask(() => this.emit('error', er)); }\n    }\n  }\n\n  [_onerror] (er) {"
    );
    globalThis.__FSMINIPASS_PATCHED = true;
  }
...
export async function createHarness({ verboseLog = false } = {}) {
  const mf = new Miniflare({
    ...
    modules: [{ type: "ESModule", path: "do-native-fs/worker/driver-do.mjs" }],
    modulesRoot: ".",
