  ...
  async installFromRegistry(fs, opts = {}) {
    ...
    const wrapRequest = (mod, name) => {
      ...
      const make = (origFn) => function (...args) {
        ...
        const tReq = Date.now();
        let released = false;
        const finish = (tag) => { inflight = Math.max(0, inflight - 1); INSTR.http.byEvent[tag] = (INSTR.http.byEvent[tag] || 0) + 1; if (!released) { released = true; release(); } };
        let req;
        try { req = origFn(...args); } catch (e) { finish("throw"); INSTR.http.reqErr++; throw e; }
        try {
          req.on("response", (res) => {
            INSTR.http.reqDone++;
            res.on("end", () => { INSTR.http.ended++; });
          ...
          req.on("timeout", () => { log2(`req#${id} TIMEOUT after ${Date.now()-tReq}ms inflight=${inflight}`); });
          // SEMAPHORE: gate the actual end() (-> fetch kickoff) behind a slot.
          if (MAXCONC > 0) {
            const origEnd = req.end.bind(req);
            req.end = (...endArgs) => {
              acquire().then(() => { try { origEnd(...endArgs); } catch (e) { finish("end-throw"); } });
              return req;
            };
          }
        } catch {}
    ...
    const sampler = setInterval(() => {
      log2(`SAMPLE inflight=${inflight} reqStart=${INSTR.http.reqStart} reqDone=${INSTR.http.reqDone} ended=${INSTR.http.ended} err=${INSTR.http.reqErr} maxInflight=${INSTR.http.inflightMax} byEvent=${JSON.stringify(INSTR.http.byEvent)}`);
    }, 2000);
    // -----------------------------------------------------------------------
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (url.pathname === "/instr") {
        let log = "";
        try { log = fs.readFileSync("/tmp/instr.log", "utf8"); } catch (e) { log = "ERR " + e.message; }
